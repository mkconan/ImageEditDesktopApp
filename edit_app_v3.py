...
def projection(img, pt_lu, pt_ru, pt_ld, pt_rd, window_height, window_width):
    ...
    射影変換
    """
    points_before_projection = np.float32([pt_lu, pt_ru, pt_ld, pt_rd])
    points_after_projection = np.float32(
        [[0.0, 0.0], [window_width, 0.0], [0.0, window_height], [window_width, window_height]]
    )
    projection_matrix = cv2.getPerspectiveTransform(points_before_projection, points_after_projection)
    dst = cv2.warpPerspective(img, projection_matrix, (window_width, window_height))
    return dst
...
def resize_in_canvas(img):
    ...
    print(f"Display Resolution: {img_resize.shape}")
...
class ImageGUI:
    ...
    def __init__(self, main):
        ...
        # 参照ボタン配置
        button0 = Button(root, text="Select", command=self.button0_clicked)
        button0.grid(row=0, column=1)
        button0.place(x=500, y=12)

        # 閉じるボタン
        close1 = Button(root, text="Close", command=self.close_clicked)
    ...
    def button0_clicked(self):
        ...
        # 顔モザイク実施するボタンの生成と配置
        self.button2 = Button(root, text="Grayscale", command=self.grayscale_clicked, width=10)
        self.button2.grid(row=0, column=3)
        self.button2.place(x=canvas_width + 50, y=90)

        self.button3 = Button(root, text="Projection", command=self.projection_clicked, width=10)
        self.button3.grid(row=0, column=3)
        self.button3.place(x=canvas_width + 50, y=120)

        self.button4 = Button(root, text="Mosaic", command=self.mosaic_clicked, width=10)
        self.button4.grid(row=0, column=3)
        self.button4.place(x=canvas_width + 50, y=150)

        self.slider = Scale(root, label="mosaic_factor", orient="h", from_=0.01, to=0.2, length=100, resolution=0.01)
        self.slider.place(x=canvas_width + 30, y=230)
        self.slider.set(0.1)

        self.button5 = Button(root, text="Reflection", command=self.remove_reflection_clicked, width=10)
        self.button5.grid(row=0, column=3)
        self.button5.place(x=canvas_width + 50, y=180)

        # 画像を保存を実施するボタンの生成と配置
        self.button6 = Button(root, text="Save", command=self.save_clicked, width=10)
        ...
        print(f"input img size : {self.input_img.shape}")
        ...
        print(f"display img size : {img2.shape}")
    ...
    def mosaic_clicked(self):
        ...
        self.id_bind2 = input_canvas.bind("<ButtonRelease-1>", self.release_event)

    def points_clicked(self, event):
        self.clicked_num += 1
        print(f"Left clicked on the canvas -> ({event.x}, {event.y})")
        if self.clicked_num <= 4:
        ...
        if self.clicked_num == 4:
            ...
            print(f"click point at {self.points}")

            input_img = cv2.imread(self.filepath)

            window_width = 400
            window_height = int(window_width * l1_distance(point_lu, point_ld) / l1_distance(point_lu, point_ru))
            projection_img = projection(input_img, point_lu, point_ru, point_ld, point_rd, window_height, window_width)
            ...
            output_canvas.create_image(0, 0, anchor="nw", image=self.out_image3)
            os.remove("projection.png")

    def rect_select(self, event):
        input_canvas.delete("rect")
        input_canvas.create_rectangle(event.x, event.y, event.x + 1, event.y + 1, outline="green", tag="rect")
    ...
    def rect_drawing(self, event):
        ...
        input_canvas.coords("rect", self.lu_point_x, self.lu_point_y, self.rd_point_x, self.rd_point_y)

    def release_event(self, event):
        img = cv2.imread(self.filepath)

        rd_x, rd_y, lu_x, lu_y = (
            int(self.rd_point_x / self.input_zoom),
            int(self.rd_point_y / self.input_zoom),
            int(self.lu_point_x / self.input_zoom),
            int(self.lu_point_y / self.input_zoom),
        )
        ...
        select_img = cv2.resize(select_img, dsize=None, fx=scale_factor, fy=scale_factor)
        select_img = cv2.resize(select_img, dsize=(rect_width, rect_height), interpolation=cv2.INTER_NEAREST)
        out_img = img.copy()
        ...
        output_canvas.create_image(0, 0, anchor="nw", image=self.out_image4)
    ...
    def remove_reflection_clicked(self):
        ...
        img = cv2.imread(self.filepath)
        #######ココ変えてね#######
        img_rmrf = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        #######################
